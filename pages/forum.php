<div class="jumbotron parallax" data-parallax="scroll" data-image-src="theme/<?php echo $_Serveur_['General']['theme'];?>/img/jumbotron.png">
    <div class="container">
        <h1> Forum </h1>
        <br/>
        <p> Bienvenue sur le forum de
            <?php echo $_Serveur_['General']['name']; ?>!
                <br> Ici, vous pourrez échanger et partager avec toute la communauté du serveur ! </p>
    </div>
</div>
<div class="container">
    <?php 
...
{ ?>
        ...
        <table class="table table-striped">
            <caption>
                <div class="row">
                    <div class="col-md-3">
                        <h3><?php echo ucfirst($fofo['nom']); ?></h3></div>
                    <?php if(isset($_Joueur_) AND $_Joueur_['rang'] == 1){ ?>
                        <div class="col-md-9" style="text-align: right;"><a href="?action=remove_forum&id=<?php echo $fofo['id']; ?>" class="btn btn-danger" style="text-align: right;">Supprimer</a></div>
                        <?php } ?>
                </div>
            </caption>
            <thead>
    ...
    $i++;
                ...
                <tbody>
                    <?php   for($i = 0; $i < count($categorie); $i++) { ?>
                        <tr>
                            <td>
                                <?php if($categorie[$i]['img'] == NULL) { ?><a href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum_categorie&id=<?php echo $categorie[$i]['id']; ?>"><i class="fa fa-comment"></i></a>
                                    <?php }
                                                                       else { ?><a href="<?php echo $_Serveur_['General']['url']; ?>?page=forum_categorie&id=<?php echo $categorie[$i]['id']; ?>"><i class="fa fa-<?php echo $categorie[$i]['img']; ?>"></i></a>
                                        <?php }?>
                            </td>
                            <td>
                                <a href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum_categorie&id=<?php echo $categorie[$i]['id']; ?>">
                                    <?php echo $categorie[$i]['nom']; ?>
                                </a>
                            </td>
                            <td>
                                <a href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum_categorie&id=<?php echo $categorie[$i]['id']; ?>">
                                    <?php echo $categorie[$i]['description']; ?>
                                </a>
                            </td>
                            <td>
                                <?php if(isset($categorie[$i]['sous-forum']) AND $categorie[$i]['sous-forum'] != NULL) {
							...
							$sousforum = $sousforum->fetch(); ?>
                                    <div class="dropdown">
                                        <button class="btn btn-default dropdown-toggle" type="button" id="sous-forum<?php echo $categorie[$i]['id']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" style="width: 99.5%;"> Il y à
                                            <?php echo count($sousforum['id']); ?> sous-forum
                                                <?php if(count($sousforum['id']) != "0") { ?><span class="caret"></span>
                                                    <?php } ?>
                                        </button>
								...
								?>
                                                    <li>
                                                        <a href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum_categorie&id=<?php echo $categorie[$i]['id']; ?>&id_sous_forum=<?php echo $sousforum['id']; ?>">
                                                            <?php echo $sousforum['nom']; ?>
                                                        </a>
                                                    </li>
                                                    <?php } ?>
                                            </ul>
                                    ...
                                    <?php }	else { ?> Il n'y a pas de sous-forum
                                        <?php } ?>
                            </td>
                            <?php if(isset($_Joueur_) AND $_Joueur_['rang'] == 1)
				{
...
if(isset($_Joueur_) AND $_Joueur_['rang'] == 1)
{
	?>
            ...
            <div style="text-align: center">
                <form action="?action=create_cat" method="post">
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="nom" id="nom" maxlength="40" class="form-control" require /> </div>
                        <div class="col-md-4">
                            <label class="control-label" for="img">Icône disponible sur : https://fontawesome.io/icons/</label>
                            <input type="text" name="img" id="img" maxlength="300" class="form-control" /> </div>
                        <div class="col-md-4">
						...
						{
							?>
                                    <option value="<?php echo $datawhile['id']; ?>">
                                        <?php echo $datawhile['nom']; ?>
                                    </option>
                                    <?php
						}
						?>
                ...
                <br>
                <hr/><a class="btn btn-primary btn-xs btn-block" role="button" data-toggle="collapse" href="#add_forum" aria-expanded="false" aria-controls="add_forum">
Ajouter un forum
</a>
                        ...
                        <form action="?action=create_forum" method="post">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="nom" id="nom" maxlength="80" class="form-control" require /> </div>
                                <br/>
                                <br>
                                <div class="row">
                                    <div class="col-md-offset-2 col-md-4">
