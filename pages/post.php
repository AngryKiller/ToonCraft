...
?>
    <div class="jumbotron parallax" data-parallax="scroll" data-image-src="theme/<?php echo $_Serveur_['General']['theme'];?>/img/jumbotron.png">
            ...
            <p>
                <ol class="breadcrumb" style="font-size: 20px;">
                    ...
                    <?php if(isset($topicd['sous_forum'])) { ?>
                        <li><a href="?page=foum_categorie&id=<?php echo $topicd['id_categorie']; ?>&id_sous_forum=<?php echo $topicd['sous_forum']; ?>"><?php $nom = $bddConnection->prepare('SELECT nom FROM cmw_forum_sous_forum WHERE id = :id'); $nom->execute(array('id' => $topicd['sous_forum'])); $nomd = $nom->fetch(); echo $nomd['nom']; ?></a></li>
                        <?php } ?>
                            <li class="active">
                                <?php echo $topicd['nom']; ?>
                            </li>
		...
		{
			?> <a class="btn btn-primary text-center" href="<?php echo $_Serveur_['General']['url']; ?>?&action=unfollow&id_topic=<?php echo $topicd['id']; ?>">Ne plus suivre cette discussion </a>
                        <?php
		}
		else
		{
			?> <a class="btn btn-primary text-center" href="<?php echo $_Serveur_['General']['url']; ?>?&action=follow&id_topic=<?php echo $topicd['id']; ?>">Suivre cette discussion</a>
                            <?php
		}
		?>
                </div>
                <div class="col-md-4"> <a class="btn btn-primary text-center" href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum_categorie&id=<?php echo $topicd['id_categorie']; if(isset($topicd['sous_forum'])) { ?>&id_sous_forum=<?php echo $topicd['sous_forum']; } ?>">Revenir à l'index de la catégorie</a> </div>
                <div class="col-md-4"> <a class="btn btn-primary text-center" href="<?php echo $_Serveur_['General']['url']; ?>?&page=forum">Revenir à l'index du forum</a> </div>
            </p>
        </div>
    </div>
    <div class="container">
        <center>
            <?php if(isset($_Joueur_) AND $_Joueur_['rang'] == 1) { ?>
                <div class="row">
                    <div class="col-lg-8">
                        <div class="dropdown">
                            <button class="btn btn-warning dropdown-toggle" type="button" id="Actions-Modération" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"> Actions de Modération <span class="caret"></span> </button>
                            <ul class="dropdown-menu list-inline" aria-labeledby="Actions-Modérations">
                                <?php 
					if($topicd['etat'] == 1)
					{
						?>
                                    <li> <a href="<?php echo $_Serveur_['General']['url']; ?>?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=4">Ouvrir la discussion</a></li>
                                    <?php
					}
					else
					{
        ...
        <h3>Sujet : <?php echo $topicd['nom']; ?></h3>
        <br/>
        <div class="row well">
            <div class="col-md-2">
                <center>
                    <!-- Div de droite où on met le profil de l'auteur -->
                    <p> <img src="http://api.craftmywebsite.fr/skin/face.php?u=<?php echo $topicd['pseudo']; ?>&s=1024&v=f" alt="" class="img-circle" width="100" height="100">
                        <hr> <strong style="font-size:15px;" class="text-center"><?php echo $topicd['pseudo']; ?></strong> </p>
                </center>
            </div>
            <div class="col-md-10">
                <!-- Contenu du topic de l'auteur -->
                <p>
                    <?php unset($contenue);
			...
			?>
                ...
                <hr>
                <p>Posté le
                    <?php  echo $topicd['jour']; ?>
                        <?php $mois = switch_date($topicd['mois']); echo $mois; ?>
                            <?php echo $topicd['annee'];?>
                                <?php if($topicd['d_edition'] != NULL) { echo 'édité le '; $d_edition = explode('-', $topicd['d_edition']); echo $d_edition[2]; echo '/' .$d_edition[1]. '/' .$d_edition[0]. ''; } ?>
                </p>
            </div>
        </div>
        <hr>
            ...
            <?php if($_Joueur_['pseudo'] == $topicd['pseudo'] OR $_Joueur_['rang'] == 1)
	{
		?>
                <div class="col-md-2">
                    <form action="?page=edit_topic" method="post">
                ...
                <div class="col-md-2">
                    <form action="?action=remove_topic" method="post">
                        <input type="hidden" name="id_topic" value="<?php echo $id; ?>" /> <a class="btn btn-round btn-default" role="button" data-toggle="collapse" href="#topic_<?php echo $id; ?>" aria-expanded="false" aria-controls="collapseExample">
	...
	{ ?>
            <div class="well" style="/*background-color: <?php if($boucle%2 == 0) { echo 'white'; } else { echo 'white'; } ?>;*/border: 1px solid lightgray; border-radius: 5px;">
                <div class="row">
                    <div class="col-md-2">
                        <div id="<?php echo $answerd['id']; ?>">
                            <!-- div de droite avec les infos joueurs -->
                            <center>
                                <!-- Div de droite où on met le profil de l'auteur -->
                                <p> <img src="http://api.craftmywebsite.fr/skin/face.php?u=<?php echo $answerd['pseudo']; ?>&s=1024&v=f" alt="" class="img-circle" width="100" height="100">
                                    <hr> <strong style="font-size:15px;" class="text-center"><?php echo $answerd['pseudo']; ?></strong> </p>
                            </center>
                        </div>
                    </div>
                    <div class="col-md-10">
				...
				?></p>
                            ...
                            <hr>
                            <p>
                                <?php echo $answerd['day']; ?>
                                    <?php $answerd['mois'] = switch_date($answerd['mois']); echo $answerd['mois']; ?>
                                        <?php echo $answerd['annee']; ?>
                                            <?php if($answerd['d_edition'] != NULL){ echo 'édité le '; $d_edition = explode('-', $answerd['d_edition']); echo '' .$d_edition[2]. '/' .$d_edition[1]. '/' .$d_edition[0]. ''; } ?>
                            </p>
                    </div>
                </div>
                <div class="row">
			...
			if($countlike > 0)
			{
				if($countlike >= 3)
				{
					...
					echo ' aiment ça ';
					?><a href="?&page=list_like&id_answer=<?php echo $answerd['id']; ?>" title="liste des j'aimes">Liste</a></div>
                <?php
				}
			...
			if($countdislike > 0)
			{
				if($countdislike > 3)
				{
					...
					echo ' n\'aiment ça';
					?><a href="?&page=list_unlike&id_answer=<?php echo $answerd['id']; ?>" title="liste des Je n'aime pas">Liste</a></div>
            <?php
				}
			...
			if($_Joueur_['pseudo'] === $answerd['pseudo'] OR $_Joueur_['rang'] == 1)
			{
				?>
                        <div class="col-md-2">
                            <form action="?page=edit_answer" method="post">
                                <input type="hidden" name="id_answer" value="<?php echo $answerd['id']; ?>" />
                        ...
                        <div class="col-md-2">
                            <form action="?action=remove_answer" method="post">
                                <input type="hidden" name="id_answer" value="<?php echo $answerd['id']; ?>" />
                                <input type="hidden" name="page" value="<?php if(isset($_GET['page_post'])) { echo $_GET['page_post']; } else { echo '1'; }?>" /> <a role="button" class="btn btn-primary" data-toggle="collapse" href="#answer_<?php echo $answerd['id']; ?>" aria-expanded="false" aria-controls="collapseExample">
	...
	?>
        <nav>
			...
			for($i = 1; $i <= $count_nbrOfPages; $i++)
			{
                ?>
                    <li>
                        <a href="?&page=post&id=<?php echo $id; ?>&page_post=<?php echo $i; ?>">
                            <?php echo $i;
                ?>
                        </a>
                    </li>
	 ...
	 {
		 ?>
            <div class="alert alert-info" role="alert">Le topic est fermé ! Aucune réponse n'est possible ! </div>
            <?php 
	 }
	 elseif($topicd['etat'] == 0)
	 {
		 ?>
                <hr/>
                <div class="separator" style="border-top: 1px solid black;"></div>
