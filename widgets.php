...
<?php for($i = 0; $i < count($_Widgets_['Widgets']); $i++) { 
	 ...
	 if((
		...
		AND ( $_Widgets_['Widgets'][$i]['type'] != 1 ) ){ ?>
		...
		<div class="panel-body <?php if($_Widgets_['Widgets'][$i]['type'] == 1) echo 'players'; ?>">
			<?php if($_Widgets_['Widgets'][$i]['type'] == 0) { ?>
				<ul class="nav nav-pills nav-stacked">
					...
					<li><a href="?&page=token"><span class="glyphicon glyphicon-euro"></span> Acheter des <?php echo $_Theme_['All']['money']; ?>s</a></li>
					<li><a href="?&action=deco"><span class="glyphicon glyphicon-off"></span> Déconnexion</a></li>
				</ul>
				<label class="label label-info label-sm">J'ai <?php echo $_Joueur_['tokens']; echo $_Theme_['All']['money']; ?>s</label>
